...
/// Dinleme oynatıcısı — seçilen soru setini eller serbest okur.
/// Soru + şıkları → doğru cevap (+ opsiyonel açıklama) → otomatik sonraki soru.
...
class _ListenPlayerScreenState extends ConsumerState<ListenPlayerScreen> {
  ...
  void initState() {
    ...
    _index = repo.listenIndex(widget.category);
    _voice = repo.ttsVoice();
    _speed = repo.ttsSpeed();
    _readExpl = repo.ttsReadExplanation();
    _initTts();
  }
  ...
  String _answerSpeech(Question q) {
    ...
    return 'Doğru cevap: $harf şıkkı.'
        '${(_readExpl && expl != null) ? ' $expl' : ''}';
  }
  ...
  Future<void> _play() async {
    ...
    while (_playing && token == _runToken && _index < qs.length) {
      ...
      await ref
          ...
          .setListenIndex(widget.category, _index);
      if (mounted) setState(() {});
    }
    ...
    if (token == _runToken && mounted) {
      if (_index >= qs.length) {
        ...
        await ref
            .read(progressRepositoryProvider)
            .setListenIndex(widget.category, 0);
      }
      ...
      body: questionsAsync.when(
        ...
        data: (all) {
          if (_questions.isEmpty) {
            _questions = widget.category.isEmpty
                ? all
                : all.where((q) => q.category == widget.category).toList();
            if (_index >= _questions.length) _index = 0;
          }
          final qs = _questions;
          if (qs.isEmpty) {
            return const Center(child: Text('Bu sette soru bulunamadı.'));
          }
          final q = qs[_index];
          return Column(
            children: [
              _ControlsBar(
                ...
                onSpeed: _setSpeed,
                onExpl: (v) async {
                  setState(() => _readExpl = v);
                  await ref
                      .read(progressRepositoryProvider)
                      .setTtsReadExplanation(v);
                },
              ),
              Expanded(
                child: ListView(
                  ...
                  children: [
                    ...
                    if (_showAnswer && _readExpl && q.explanation != null) ...[
...
/// Ses + hız + açıklama kontrolleri (üst bar).
class _ControlsBar extends StatelessWidget {
  ...
  final ValueChanged<bool> onExpl;
  const _ControlsBar({
    required this.voices,
    required this.voice,
    required this.speed,
    required this.readExpl,
    required this.onVoice,
    required this.onSpeed,
    required this.onExpl,
  });
      ...
      child: Column(
        ...
        children: [
          ...
          SwitchListTile(
            ...
            secondary: const Icon(Icons.notes_outlined, size: 20),
